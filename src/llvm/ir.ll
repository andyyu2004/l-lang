...
basic_blockbb0:
  ...
  %cast_malloc_ptr = bitcast { %"Node<>", i32 }* %box to i8*
  ...
  call void @"rc_retain<Node<>>"(%"Node<>"** %tmp4)
...
basic_blockbb2:                                   ; preds = %basic_blockbb1
  ...
  call void @"rc_retain<Node<>>"(%"Node<>"** %n)
...
define void @"rc_retain<Node<>>"(%"Node<>"** %0) {
rc_retain_start:
  %alloc_str = alloca [17 x i8]
  store [17 x i8] c"rc_retain_count\0A\00", [17 x i8]* %alloc_str
  %cast_str = bitcast [17 x i8]* %alloc_str to i8*
  %print_str = call i32 (i8*, ...) @printf(i8* %cast_str)
  %load_box = load %"Node<>"*, %"Node<>"** %0
  %cast_malloc_ptr = bitcast %"Node<>"* %load_box to i8*
  %print_malloc_addr = call {} @print_addr(i8* %cast_malloc_ptr)
  %rc_retain_box_cast = bitcast %"Node<>"* %load_box to { %"Node<>", i32 }*
  %rc = getelementptr inbounds { %"Node<>", i32 }, { %"Node<>", i32 }* %rc_retain_box_cast, i32 0, i32 1
