...
pub macro declare_arena([$($a:tt $name:ident: $ty:ty,)*]) {
    ...
    pub trait ArenaAllocatable<'tcx, C = lc_arena::IsNotCopy>: Sized {
    ...
    impl<'tcx, T: Copy> ArenaAllocatable<'tcx, lc_arena::IsCopy> for T {
        #[inline]
        #[allow(clippy::mut_from_ref)]
        fn allocate_on<'a>(self, arena: &'a Arena<'tcx>) -> &'a mut Self {
    ...
    $(
        impl<'tcx> ArenaAllocatable<'tcx, lc_arena::IsNotCopy> for $ty {
            #[inline]
            fn allocate_on<'a>(self, arena: &'a Arena<'tcx>) -> &'a mut Self {
