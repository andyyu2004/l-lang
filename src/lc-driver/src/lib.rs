...
use lc_core::{GlobalCtx, TyCtx};
use lc_error::{ErrorFormat, ErrorReported, LResult};
use lc_index::IndexVec;
use lc_ir::{PkgId, Resolutions};
use lc_lex::{Lexer, TokenIterator, TokenStream};
use lc_meta::PkgMetadata;
use lc_parse::Parser;
use lc_resolve::{Resolver, ResolverArenas};
...
pub struct Driver<'tcx> {
    ...
    core_arenas: lc_core::Arena<'tcx>,
    ir_arena: lc_astlowering::Arena<'tcx>,
    ...
    ($self:expr, $ret:expr) => {{
        if $self.sess.has_errors() {
            ...
            if $self.sess.opts.error_format == ErrorFormat::Text {
                ...
                if warnings > 0 {
                    e_yellow_ln!("{} warning{} emitted", warnings, lc_util::pluralize!(warnings));
                }
                e_red_ln!("{} error{} emitted", errc, lc_util::pluralize!(errc));
            }
            Err(ErrorReported)
        } else {
            Ok($ret)
...
impl<'tcx> Driver<'tcx> {
    ...
    pub fn parse_expr(&self) -> Option<P<lc_ast::Expr>> {
        let mut parser = Parser::new(&self.sess);
    ...
    pub fn parse_macro(&self) -> Option<lc_ast::Macro> {
        let mut parser = Parser::new(&self.sess);
...
impl<'tcx> Driver<'tcx> {
    pub fn gen_tir(&'tcx self) -> LResult<lc_tir::Prog<'tcx>> {
        self.with_tcx(lc_mirgen::build_tir)?
    }
}
