    ...
    W: Write,
{
    ...
    pub fn fmt_expr(&mut self, expr: &tir::Expr) -> fmt::Result {
        match &expr.kind {
            ...
            tir::ExprKind::Tuple(xs) => indent!(self, "({})", lc_util::join2(xs.iter(), ",")),
            ...
            tir::ExprKind::Closure { upvars: _, body } =>
                indent!(self, "(λ({}) {})", lc_util::join2(body.params.iter(), ","), body),
    ...
    fn fmt_call(&mut self, f: &tir::Expr, args: &[tir::Expr]) -> fmt::Result {
        match args.len() {
            0 => indent!(self, "({})", f),
            _ => indent!(self, "({} {})", f, lc_util::join2(args.iter(), " ")),
        }
    }
