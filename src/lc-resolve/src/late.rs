    ...
    fn resolve_item(&mut self, item: &'ast Item) {
        match &item.kind {
            ItemKind::Fn(_, g, _) | ItemKind::TypeAlias(g, _) =>
                self.with_generics(g, |r| lc_ast::walk_item(r, item)),
            ItemKind::Enum(g, _) | ItemKind::Struct(g, _) => self.resolve_adt(g, item),
            ItemKind::Extern(..) => lc_ast::walk_item(self, item),
            ItemKind::Mod(module) =>
                self.with_module(item.ident, |this| lc_ast::walk_module(this, module)),
            ItemKind::Use(..) => {}
            ItemKind::Trait { generics, items } => self.with_generics(generics, |r| {
                r.with_self(item.id, |r| {
                    for item in items {
                        lc_ast::walk_assoc_item(r, item);
                    }
                })
            }),
    ...
    fn resolve_assoc_item(&mut self, item: &'ast AssocItem) {
        match &item.kind {
            ...
            AssocItemKind::Fn(_, generics, _) =>
                self.with_generics(generics, |this| lc_ast::walk_assoc_item(this, item)),
        }
    }

    fn resolve_adt(&mut self, generics: &'ast Generics, item: &'ast Item) {
        self.with_generics(generics, |this| lc_ast::walk_item(this, item))
    }
}

impl<'a, 'ast> lc_ast::Visitor<'ast> for LateResolver<'a, '_, 'ast> {
    fn visit_block(&mut self, block: &'ast Block) {
        self.with_val_scope(|this| lc_ast::walk_block(this, block));
    }

    /// create a scope for fn parameters
    fn visit_fn(&mut self, sig: &'ast FnSig, body: Option<&'ast Expr>) {
        self.with_val_scope(|this| lc_ast::walk_fn(this, sig, body));
    }

    fn visit_closure(&mut self, name: Option<Ident>, sig: &'ast FnSig, body: &'ast Expr) {
        self.with_val_scope(|this| lc_ast::walk_closure(this, name, sig, body));
    }
