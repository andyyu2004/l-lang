...
impl<'a> Parse<'a> for FnTyParser {
    ...
    fn parse(&mut self, parser: &mut Parser<'a>) -> ParseResult<'a, Self::Output> {
        ...
        let output = parser.accept(TokenKind::RArrow).map(|_| parser.parse_ty(false));
        Ok((inputs, output))
    }
}
