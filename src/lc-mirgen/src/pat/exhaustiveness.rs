...
use ir::DefId;
use lc_core::ty::{tls, Const, Substs, SubstsRef, Ty, TyKind};
use lc_span::Span;
...
impl<'p, 'tcx> MatchCtxt<'p, 'tcx> {
    pub(crate) fn check_match(&self, span: Span, scrut: &tir::Expr<'tcx>, arms: &[tir::Arm<'tcx>]) {
...
impl<'p, 'tcx> Display for Witness<'p, 'tcx> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "{}", lc_util::join(&self.pats, ","))
    }
}
...
impl<'p, 'tcx> Display for Fields<'p, 'tcx> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(f, "{}", lc_util::join2(self.pats, ", "))
    }
}
